  ...
  async function createContent(event) {
    try {
      ...
      await ContentsApi().createContent(
        req.nome.value, req.midia.value, req.titulo.value, Number(req.identificacao.value)
      ).then(data => {
        return data.json()
      }).then(res => {
        setContents([...contents, {
          id: res.contentId,
          nome: req.nome.value,
          midia: req.midia.value,
          titulo: req.titulo.value,
          identificacao: Number(req.identificacao.value)
        }])
  ...
  async function updateContent(event) {
    try {
      ...
      await ContentsApi().updateContent(
        selectedContent.id, req.nome.value, req.midia.value, req.titulo.value, Number(req.identificacao.value)
      )
